    ...
    async def create_wallets(
        ...
        try:
            ...
            if self.privy:
                ...
                # Wallets are created by the frontend Privy SDK (createOnLogin).
                # During setup, we just verify the connection works.
                if progress_callback:
                    progress_callback(60)

                # Store placeholder — actual wallet addresses are fetched
                # per-user from Privy linked_accounts during auth sync.
                await self.db.set_config("wallet_provider", "privy")
