"""
Opportunity Detector for Asgard Basis.
...
from shared.config.assets import Asset, get_mint, get_asset_metadata
from shared.config.assets import AssetMetadata
...
from bot.venues.hyperliquid.funding_oracle import HyperliquidFundingOracle, FundingPrediction
...
class OpportunityDetector:
    """
    Detects funding rate arbitrage opportunities for SOL.
    ...
    # Supported assets (SOL-only strategy)
    ALLOWED_ASSETS = [Asset.SOL]
    ...
    async def scan_opportunities(
        ...
        for asset in assets:
            try:
                ...
                if hl_funding:
                    from shared.models.funding import FundingRate as ModelFundingRate
                    # Oracle funding_rate is hourly; model expects 8hr rate
                    model_funding = ModelFundingRate(
                        timestamp=datetime.utcnow(),
                        coin="SOL",
                        rate_8hr=Decimal(str(hl_funding.rate_8hr)),
                    )
                else:
                    model_funding = None
    ...
    async def _create_opportunity(
        ...
        # Calculate funding APY on deployed capital (absolute value since shorts receive payment)
        # Funding is earned on the full short notional = capital × leverage
        funding_apy = abs(Decimal(str(current_funding.rate_annual))) * self.leverage
        ...
        # Oracle predicted_rate is hourly; model expects 8hr rate
        predicted_funding = FundingRate(
            timestamp=datetime.utcnow(),
            coin="SOL",
            rate_8hr=Decimal(str(prediction.predicted_rate * 8)),
        )
    ...
    async def calculate_total_apy(
        ...
        # Calculate components (all per deployed capital)
        # Funding is earned on full short notional = capital × leverage
        funding_apy = abs(funding_rate) * self.leverage
