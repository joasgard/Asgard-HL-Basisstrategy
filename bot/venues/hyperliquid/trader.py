...
class HyperliquidTrader:
    ...
    def __init__(
        ...
        Args:
            ...
            wallet_address: EVM wallet address for this user (used if signer not provided)
        ...
        # Resolve wallet address: explicit param > signer's address > settings fallback
        self.wallet_address = wallet_address

        if self.signer is not None:
            # Signer provided — use its wallet address if we don't have one
            if not self.wallet_address:
                self.wallet_address = self.signer.wallet_address
        elif self.wallet_address:
            # No signer but wallet_address provided — create per-user signer
            try:
            ...
            # No signer, no wallet — fall back to global settings
            try:
    ...
    async def open_short(
        ...
        if deposited_balance < required_margin * 0.95:  # 5% buffer
            logger.error(
                f"Insufficient deposited balance: ${deposited_balance:.2f} USDC "
                f"(need ${required_margin:.2f}). "
                f"Deposit funds to Hyperliquid clearinghouse first."
            )
            return OrderResult(
                success=False,
    ...
    async def get_deposited_balance(self) -> float:
        ...
        try:
            ...
            # USDC is typically the first asset in the balances array
            balances = state.get("balances", [])
            for balance in balances:
                if balance.get("coin") == "USDC":
                    return float(balance.get("total", 0)) / 1_000_000  # Convert from 6 decimals
    ...
    async def transfer_spot_to_perp(self, amount_usd: float) -> bool:
        ...
        This is an internal Hyperliquid transfer (spot → perp), NOT a bridge
        deposit from Arbitrum. For bridging USDC from Arbitrum, use
        HyperliquidDepositor.deposit().
        ...
        try:
            ...
            raw_amount = str(int(amount_usd * 1_000_000))
            
            logger.info(f"Transferring {amount_usd} USDC from spot to perp on Hyperliquid")
            
            # Sign deposit
            signed_transfer = await self.signer.sign_spot_transfer(usdc_amount=raw_amount)
            ...
            payload = {
                "action": {
                    "type": "spotTransfer",
                    "token": "USDC",
                    "amount": signed_transfer.usdc_amount,
            ...
            if response.get("status") == "ok":
                logger.info(f"Successfully transferred {amount_usd} USDC (spot → perp)")
                return True
            else:
