"""
Position Manager for Delta Neutral Arbitrage.
...
from src.config.assets import Asset, get_asset_metadata, AssetMetadata
from src.config.settings import get_settings, get_risk_limits
from src.core.fill_validator import FillValidator, FillInfo
from src.core.price_consensus import PriceConsensus, ConsensusResult
from src.models.common import Protocol, TransactionState, ExitReason
...
class PositionManager:
    ...
    async def _check_wallet_balance(self, opportunity: ArbitrageOpportunity) -> bool:
        ...
        try:
            ...
            # Don't fail if we can't check, assume OK for testing
            return True
        
        # Check Arbitrum balance (need ETH for gas)
        try:
            eth_balance = await self.arbitrum_client.get_balance()
            # Need at least 0.01 ETH for gas (~$20-30)
            if eth_balance < 0.01:
                logger.warning(f"Insufficient ETH balance: {eth_balance}")
                return False
        except Exception as e:
